...
- Prefer completing Watch pairing, Mac bridge install, and Watch-to-Mac delivery rather than handing back a walkthrough when asked to make it work. Physical Watch acceptance remains `blocked:external` until a new memo lands on the Mac.
- Want one user-facing macOS companion: the existing CodexWatch binary branded **Codex Watch** (with a space), pairing/Speech/transcription/inbox, Signal Spine Watch visual language, hierarchy-first polish rather than extra glass; not a second daemon or `/tmp` sidecar.
- Treat hardware status only as `physical-watch-proven`, `blocked:external`, or `unverified`.
- Codex Watch protocol identity (bundle IDs, Bonjour, LaunchAgent, state root, TLS CN) replaced Voice Inbox. Re-pair on-wrist after install. Mac Reset must not wipe Watch Keychain from the Mac.
...
- Watch app bundle ID is `ai.rsitech.codexwatch`; physical install targets Apple Watch Ultra 2 and needs the companion iPhone CoreDevice tunnel held, or preflight stays `SUPPORTING_PHONE_UNAVAILABLE`.
- Mac intake is LaunchAgent `ai.rsitech.codexwatch.bridge` advertising Bonjour `_codexwatch._tcp` as **Codex Watch**; the installed UI+listener is `$HOME/Library/Application Support/CodexWatch/Service/CodexWatch.app` (same signed binary as the agent). Leave legacy `com.rsitech.codex-watch-bridge` on loopback alone; do not use `/tmp` sidecars.
