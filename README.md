...
- Source verification means package tests, the Watch simulator build, bridge smokes,
  privacy metadata, and release packaging pass for the published source.
- The exact `v0.1.0` macOS bridge download is verified: its published
...
- Physical Apple Watch capture and the complete Watch-to-Mac workflow remain
  blocked by the physical Watch device connection while the connected Watch's CoreDevice tunnel is
